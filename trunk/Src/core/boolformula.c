...
extern inline lit boolformula_lit_from_var (var v)
{
  assert (v > 0);
  return v;
}

extern inline var boolformula_var_from_lit (lit l)
{
  assert (l != 0);
  return l > 0 ? l : -l;
}

extern inline lit boolformula_lit_complement (lit l)
{
  assert (l != 0);
  return -l;
}

extern inline bool boolformula_positive_lit (lit l)
{
  assert (l != 0);
  return l > 0;
}
 ...
 */
extern inline boolformula_t *boolformula_disjunction_unit (void)
{
...
extern inline boolformula_t *boolformula_disjunction_new (uscalar_t length)
{
	boolformula_t* ret=malloc(sizeof(boolformula_t));
 ...
 */
extern inline boolformula_t *boolformula_conjunction_unit (void)
{
	boolformula_t* ret=malloc(sizeof(boolformula_t));
	ret->d.v=vector_new(0);
	ret->t=conjunct;
...
extern inline boolformula_t *boolformula_conjunction_new (uscalar_t length)
{
	boolformula_t* ret=malloc(sizeof(boolformula_t));
	ret->t=conjunct;
	ret->d.v=vector_new(length);
...
extern inline boolformula_t *boolformula_literal_new (lit l)
{
	boolformula_t* ret=malloc(sizeof(boolformula_t));
	ret->d.l=l;
	ret->t=literal;
...
extern inline boolformula_t *boolformula_add (boolformula_t *f,
		boolformula_t *g)
{
	assert(f->t!=literal);
	vector_add (f->d.v, g);
...
extern inline boolformula_t *boolformula_set (boolformula_t *f, uscalar_t idx,
		boolformula_t *g)
{
	assert(f->t!=literal);
...
extern inline void boolformula_free (boolformula_t *f)
{
	if(f->ref>1){
		f->ref--;
		return;
...
extern inline boolformula_t* boolformula_neg (boolformula_t * f){
	int num_of_subformulae;
	int i;
	switch(f->t){
		case disjunct:
...
extern inline boolformula_t *boolformula_to_cnf (boolformula_t * f, scalar_t num_var){
...
extern inline enum type boolformula_get_type (boolformula_t * f){
        assert (f->t == literal || f->t == conjunct || f->t == disjunct);
	return f->t;
}

extern inline uscalar_t boolformula_get_length (boolformula_t * f){
	assert(f->t == conjunct || f->t == disjunct);
        return vector_length (f->d.v);
}

extern inline boolformula_t *boolformula_get_subformula (boolformula_t * f, uscalar_t idx){
	assert(f->t == conjunct || f->t == disjunct);
	return vector_get(f->d.v, idx);
}

extern inline lit boolformula_get_value (boolformula_t * f){
	assert(f->t==literal);
	return f->d.l;
}
...
extern inline boolformula_t *boolformula_copy(boolformula_t * f){
