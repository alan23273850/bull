...
/*
* ./learn <xxxxx.cnf    Learn the formula in xxxxx.cnf using CDNF algorithm
* ./learn <xxxxx.cnf 0  Learn the formula in xxxxx.cnf using CDNF+ algorithm
* ./learn <xxxxx.cnf n  Learn the formula in xxxxx.cnf using CDNF++ algorithm with initial variable number n
*/
int main(int argc, char *argv[]){
	...
	if(argc==2){
		mode =	atoi( argv[1] );
		if(mode != CDNF && mode != CDNF_Plus && mode != CDNF_PlusPlus && mode != CDNF_Plus3)
			print_help();
...
extern inline sat_result_t* satisfible(boolformula_t * f){
	int tgtSize=0;
#ifdef DEBUG
	fprintf(stderr,"\n Ori:");
	boolformula_print(f);
...
inline boolformula_t *remap(boolformula_t * f, int* map){
	...
	case literal:
		if(f->d.l>0)
			ret=boolformula_literal_new(map[f->d.l]);
		else
			ret=boolformula_literal_new(-map[-f->d.l]);
		break;
	}
